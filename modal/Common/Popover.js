...
var Popover = React.createClass({
                                    ...
                                    getDefaultProps() {
                                        return {
                                            isVisible: false,
                                            displayArea: new Rect(0, 0, SCREEN_WIDTH,
                                                                  SCREEN_HEIGHT),
                                    ...
                                    measureContent(x) {
                                        ...
                                        this.setState(Object.assign(geom,
                                            ...
                                            // Once state is set, call the showHandler so it can
                                            // access all the geometry from the state
                                    ...
                                    computeTopGeometry({displayArea, fromRect, contentSize, arrowSize}) {
                                        var popoverOrigin = new Point(
                                            Math.min(displayArea.x + displayArea.width
                                                     - contentSize.width,
                                                     Math.max(displayArea.x,
                                                              fromRect.x + (fromRect.width
                                                                            - contentSize.width)
                                                                           / 2)),
                                            fromRect.y - contentSize.height - arrowSize.height);
                                        var anchorPoint = new Point(
                                            fromRect.x + fromRect.width / 2.0, fromRect.y);
                                    ...
                                    computeLeftGeometry({displayArea, fromRect, contentSize, arrowSize}) {
                                        var popoverOrigin = new Point(
                                            ...
                                            Math.min(displayArea.y + displayArea.height
                                                     - contentSize.height,
                                                     Math.max(displayArea.y,
                                                              fromRect.y + (fromRect.height
                                                                            - contentSize.height)
                                                                           / 2)));
                                        var anchorPoint = new Point(fromRect.x,
                                                                    fromRect.y + fromRect.height
                                                                                 / 2.0);
                                            ...
                                            Math.min(displayArea.y + displayArea.height
                                                     ...
                                                     Math.max(displayArea.y,
                                                              fromRect.y + (fromRect.height
                                                                            - contentSize.height)
                                                                           / 2)));
                                        var anchorPoint = new Point(fromRect.x + fromRect.width,
                                                                    fromRect.y + fromRect.height
                                                                                 / 2.0);
                                    ...
                                    computeAutoGeometry({displayArea, contentSize}) {
                                        ...
                                        for (var i = 0; i < placementsToTry.length; i++) {
                                            ...
                                            var geom = this.computeGeometry(
                                                {contentSize: contentSize, placement: placement});
                                    ...
                                    getArrowDynamicStyle() {
                                        ...
                                        // Create the arrow from a rectangle with the appropriate
                                        // borderXWidth set A rotation is then applied dependending
                                        // on the placement Also make it slightly bigger to fix a
                                        // visual artifact when the popover is animated with a
                                        // scale
                                    ...
                                    getTranslateOrigin() {
                                        ...
                                        return new Point(anchorPoint.x - popoverCenter.x,
                                    ...
                                    componentWillReceiveProps(nextProps) {
                                        ...
                                        if (willBeVisible !== isVisible) {
                                            if (willBeVisible) {
                                                // We want to start the show animation only when
                                                // contentSize is known so that we can have some
                                                // logic depending on the geometry
                                                this.setState(
                                                    {contentSize: {}, isAwaitingShow: true});
                                            } else {
                                    ...
                                    _startAnimation({show}) {
                                        var handler = this.props.startCustomAnimation
                                                      || this._startDefaultAnimation;
                                        handler({
                                                    show,
                                                    doneCallback: () => this.setState(
                                                        {isTransitioning: false})
                                                });
                                    ...
                                    _startDefaultAnimation({show, doneCallback}) {
                                        ...
                                        var commonConfig = {
                                            ...
                                            easing: show ? Easing.out(Easing.back()) : Easing.inOut(
                                                Easing.quad),
                                        }
                                    ...
                                    _getDefaultAnimatedStyles() {
                                        ...
                                        if (typeof this.props.startCustomAnimation
                                            !== 'undefined') {
                                            return null;
                                        }
                                    ...
                                    _getExtendedStyles() {
                                        ...
                                        [this._getDefaultAnimatedStyles(), this.props].forEach(
                                            (source) => {
                                                if (source) {
                                    ...
                                    render() {
                                        ...
                                        var arrowColor = StyleSheet.flatten(
                                        ...
                                        // Special case, force the arrow rotation even if it was
                                        // overriden
                                        var arrowStyle = [styles.arrow, arrowDynamicStyle,
                                                          arrowColorStyle, ...extendedStyles.arrow];
                                        var arrowTransform = (StyleSheet.flatten(
                                            arrowStyle).transform || []).slice(0);
                                        arrowTransform.unshift(
                                            {rotate: this.getArrowRotation(placement)});
                                        arrowStyle = [...arrowStyle, {transform: arrowTransform}];
                                        var contentMarginRight = this.props.contentMarginRight
                                            ? this.props.contentMarginRight : 0;
                                        return (
                                            <TouchableWithoutFeedback onPress={this.props.onClose}>
                                                <View style={[styles.container, contentSizeAvailable
